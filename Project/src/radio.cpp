...
#define CE_DDR		        RF_CE_DDR
#define CSN_DDR		        SOFTSPI_RF_CS_DDR
#define CE_PORT		        RF_CE_PORT
#define CSN_PORT	        SOFTSPI_RF_CS_PORT
#define CE_PIN		        RF_CE_PIN
#define CSN_PIN		        SOFTSPI_RF_CS_PIN
...
static uint8_t get_status() {
    ...
    softspi_transfer(&nop, 1, &status, 1);
...
static uint8_t set_register(radio_register_t reg, uint8_t* value, uint8_t len) {
    ...
    softspi_write_uint8(W_REGISTER | (REGISTER_MASK & reg));
    softspi_write(value, len);
...
static uint8_t get_register(radio_register_t reg, uint8_t* buffer, uint8_t len) {
    ...
    softspi_write_uint8(R_REGISTER | (REGISTER_MASK & reg));
    softspi_read(buffer, len);
...
static void send_instruction(uint8_t instruction, uint8_t* data, uint8_t* buffer, uint8_t len) {
    ...
    softspi_write_uint8(instruction);
    // pass in args
    if (len > 0) {
        if (buffer == NULL)	//
            softspi_write(data, len);
        else
            softspi_transfer(data, len, buffer, len);
    }
    // resynch SPI
    CSN_HIGH();
}
...
static void configure_registers() {
    ...
    // Set to use 2.4 GHz channel 111.
    value = CHANNEL;
    ...
    value = _BV(EN_DPL) | _BV(EN_DYN_ACK) | _BV(EN_ACK_PAY);
...
RADIO_TX_STATUS Radio_Transmit(radiopacket_t* payload, uint8_t len, RADIO_TX_WAIT wait) {
...
RADIO_RX_STATUS Radio_Receive(radiopacket_t* buffer, uint8_t *len) {
    ...
    send_instruction(R_RX_PL_WID, NULL, len, 1);
    ...
    if (doMove) {
        ...
        send_instruction(R_RX_PAYLOAD, (uint8_t*) buffer, (uint8_t*) buffer, *len);
